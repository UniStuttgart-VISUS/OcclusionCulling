...
bool gEnableCullingDX11 = true;
bool gEnableFCullingDX11 = true;
bool gViewDepthBufferDX11 = false;
bool gViewBoundingBoxDX11 = false;
bool gEnableTasksDX11 = true;
bool gPipelineDX11 = false;
float gCameraDX11[] = { 27.0f, 2.0f, 47.0f, 41.0f, 8.0f, -50.0f };
float gOccluderSizeThresholdDX11 = 1.5f;
float gOccludeeSizeThresholdDX11 = 0.01f;
UINT gDepthTestTasksDX11 = 20;
UINT gCameraTourDX11 = 0;
UINT gFramesForEvaluationDX11 = 0;
...
//    mpOcclusionMatchGPUResolution(NULL),
	...
	mRasterizeTime(0.0),
	mOccluderSizeThreshold(gOccluderSizeThresholdDX11),
	mNumCulled(0),
	mNumVisible(0),
	mNumOccludeeTris(0),
	...
	mDepthTestTime(0.0),
	mOccludeeSizeThreshold(gOccludeeSizeThresholdDX11),
	mTotalCullTime(0.0),
	mEnableCulling(true),
	mEnableFCulling(gEnableFCullingDX11),
	mViewDepthBuffer(gViewDepthBufferDX11),
	mViewBoundingBox(gViewBoundingBoxDX11),
	mEnableTasks(gEnableTasksDX11),
	mPipeline(gPipelineDX11),
	mCameraTour(gCameraTourDX11),
	mEvaluationName(gEvaluationNameDX11),
...
{	
	// Set commandLine camera position
	mCamera[0] = gCameraDX11[0];
	mCamera[1] = gCameraDX11[1];
	mCamera[2] = gCameraDX11[2];
	mCamera[3] = gCameraDX11[3];
	mCamera[4] = gCameraDX11[4];
	mCamera[5] = gCameraDX11[5];

	if (gCameraTourDX11 == 1)
	{
		mCamera[0] = 38.6;
		mCamera[1] = 52.5;
		mCamera[2] = 166.6;
	...
	std::vector<std::string> mEvaluationResults;
...
//    mpOcclusionMatchGPUResolution->SetCheckboxState(state);
	...
	// Set commandLine camera position or use default values
	mpCamera->SetPosition(mCamera[0], mCamera[1], mCamera[2]);
	mpCamera->LookAt(mCamera[3], mCamera[4], mCamera[5]);
	mpCamera->Update();
...
void MySample::Render(double deltaSeconds)
{
	...
	if (mFramesDone == 0)
	{
		if (gEnableCullingDX11 == false)
		{
			...
			mpCullingCheckBox->SetCheckboxState(CPUT_CHECKBOX_UNCHECKED);
			mEnableCulling = false;
		}
		if (gEnableFCullingDX11 == false)
		{
			mpDBR->ResetInsideFrustum();
			mpAABB->ResetInsideFrustum();
		}
	...
	else if (mFramesDone < mFramesForEvaluation)
	{
		AddFrameToEvaluation(mFramesDone, CPUTGuiControllerDX11::GetController()->GetFPS(), mNumCulled, mNumVisible, mNumDrawCalls, mRasterizeTime,  mDepthTestTime, mNumOccludeeCulledTris, mNumOccludeeVisibleTris, mNumOccludersR2DB);
		
		if (mCameraTour == 1)
		{
			float oneStep = PI * 2 / mFramesForEvaluation;
			float radius = 100;
			mpCamera->SetPosition(mCamera[0] + radius*sin(oneStep*mFramesDone), mCamera[1], mCamera[2] + radius*cos(oneStep*mFramesDone));
			mpCamera->LookAt(mCamera[3], mCamera[4], mCamera[5]);
			mpCamera->Update();
		}
		else if (mCameraTour == 2)
		{
			float oneStep = PI * 2 / mFramesForEvaluation;
			float radius = 1.5*(mCamera[3]-mCamera[0]);
			mpCamera->SetPosition(mCamera[3] + radius*sin(oneStep*mFramesDone), mCamera[1], mCamera[5] + radius*cos(oneStep*mFramesDone));
			mpCamera->LookAt(mCamera[3], mCamera[4], mCamera[5]);
			mpCamera->Update();
		}
		mFramesDone++;
...
void MySample::AddFrameToEvaluation(UINT mFramesDone, float mFPS, UINT mNumCulled, UINT mNumVisible, UINT mNumDrawCalls, float mRasterizeTime, float mDepthTestTime, UINT mNumOccludeeCulledTris, UINT mNumOccludeeVisibleTris, UINT mNumOccludersR2DB)
{
	if (mFramesDone == 1)
	{
		mEvaluationResults.push_back("Frame-ID<FPS<Models Culled<Models Visible<Draw Calls<Culling time<Rasterize Time<DepthTest Time<Culled Tries<Visible Tries<Depth rasterized models\n");
		mEvaluationResults.push_back("F-ID<FPS<M. Culled<M. Visible<Draw Ca.<Culling t<R t<DT t<Culled Tr.<Visible Tr.<DRM\n");
	}
	std::stringstream ss;
	ss << mFramesDone << " " << mFPS << " " << mNumCulled << " " << mNumVisible << " " << mNumDrawCalls << " " << mTotalCullTime << " " << mRasterizeTime << " " << mDepthTestTime << " " << mNumOccludeeCulledTris << " " << mNumOccludeeVisibleTris << " " << mNumOccludersR2DB << "\n";
	std::string s = ss.str();
	mEvaluationResults.push_back(s);
}

void MySample::PrintEvaluation()
{
	std::ofstream outfile;
	outfile.open(L"Evaluation/" + mEvaluationNr + L"/" + mEvaluationName);
	
	for (int i = 0; i < mEvaluationResults.size(); i++) {
		outfile << mEvaluationResults.at(i);
	}
}
