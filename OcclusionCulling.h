...
private:
	...
	float				mCamera[6];
	UINT				mCameraTour;
	std::wstring		mEvaluationName;
	std::wstring		mEvaluationNr;
	UINT				mFramesForEvaluation;
	UINT				mFramesDone;
...
public:
	...
	void AddFrameToEvaluation(UINT mFramesDone, float mFPS, UINT mNumCulled, UINT mNumVisible, UINT mNumDrawCalls, float mRasterizeTime, float mDepthTestTime, UINT mNumOccludeeCulledTris, UINT mNumOccludeeVisibleTris, UINT mNumOccludersR2DB);
